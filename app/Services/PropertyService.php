   ...
   public function getRecentProperties()
    {
        ...
        $properties = Property::where('availability','Y')->with('offer')/*->where('created_at','>=',$date)*/->orderBy('id','DESC')->paginate(2);
    ...
    function propertySingleType($id){
       /* dump($id);*/
        ...
        $language2 =  App::getLocale();
     /*   $language2 =  Session::get('language');*/
    ...
    function getPropertytitle($text) {
        $string = str_replace("+", " ", $text);
        return $string;
    }
    function getdestinationTitle($name){
        $string = str_replace("+", " ", $name);
        return $string;
    }

    function getBlogTitle($name){
        $string = str_replace("+", " ", $name);
        return $string;
    }

      function getPropTypeTitle($name){
        $string = str_replace("+", " ", $name);
        return $string;
    }

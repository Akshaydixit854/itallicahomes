...
class IndexController extends Controller
{
    ...
    {   
      /*  $language =  App::getLocale();
       ...
       /* dd($properties);*/
        $active = 'home';
        return view('ui.index',compact('properties','active','propertyTypes','property_count'));
    }
...
. '&maptype=hybrid'
.'&zoom=18&key=AIzaSyC808NpIO-MZ_C-nXv21zGFDKC1OUm_MkQ&size=510x250&markers=size:tiny|color:red|'
. $property->property_location_latitude. ",". $property->property_location_longitude);*/
...
$st = \Storage::put('public/ui/images/pdf_template/', $json);*/
/*dd($st);*/
...
/*        dd($property);*/
    ...
    public function contactStore(Request $request)
    {
        ...
        Mail::send('ui.contact_enquiry', ['data' => $data,'site_details'=>array()], function ($mail) use ($data) {
            $mail->to('info@italica.de',  'Italica')->subject('Contact Us');
        });
...
/*
        LanguageLine::create([
            'group' => 'propertyMessage',
            'key' => 'property_message_'.$propertyEnquiry->property_id,
            'text' => [
                'en' => $request->get('message'),
                'it' => $request->get('message'),
                'de' => $request->get('message')
            ],
          ]);*/
     ...
     /*   dump($property);*/
        ...
        Mail::send('ui.email_enq', ['data' => $data,'site_details'=>array()], function ($mail) use ($data) {
            $mail->to($data['email'],  $data['name'])->subject('Property Enquiry');
        });

    
       Mail::send('ui.email_enq', ['data' => $data,'site_details'=>array()], function ($mail) use ($data) {
           $mail->to('info@italica.de',  'Italica')->subject('Property Enquiry');
       });
    ...
    public function contactFaq(Request $request)
    {
       ...
       $data = array(
        ...
        Mail::send('ui.faq_email', ['data' => $data,'site_details'=>array()], function ($mail) use ($data) {
            $mail->to('info@italica.de',  'Italica')->subject('Italica FAQ');
        });
    ...
    {   
      ...
      /*  dump($id,$type);*/
       ...
       /*  if($id == 'type'){
            $request->property_type_id = $type;
            $properties->where('type_id',$type);
        }*/
        if($id == 'type'){
            $proptype='';
      ...
      /*        dd($propType_title,$proptype);*/
   /*   dump($proptype,$proptype->id);*/
            if($proptype){
        ...
        //                     ->select('locations.location_name','properties.location_id', DB::raw('count(*) as total'))
    ...
    public function update_language($locale,Request $request){
     ...
     /*   $match_array10 = array('destination','ort','luoghi');*/
       ...
       if( $single_dest && $name!='' && !$search_url && !is_numeric($name)) {
      /*  dd('dsaadsad');*/
        $id = '';
        if (is_numeric($name)) 
        { 

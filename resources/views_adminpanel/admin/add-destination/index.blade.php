                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header bg-pink">
                            ...
                            <a href="{{route('add-destination.create')}}" class="btn btn-info m-t-15 waves-effect">Add New</a>
                                ...
                                <table class="display nowrap table table-bordered table-striped table-hover dataTable js-exportable" id="example">
                                    ...
                                    <tbody>
                                        @if (count($destinations) > 0)
                                            ...
                                            <tr>
                                            	<td>{{ $key + 1 }}</td>
                                                <td><a target="_blank" href="{{url('')}}/images/destination-covers/{{$row->cover_image}}" class="table-prev-img"> <img  src="{{url('')}}/images/destination-covers/{{$row->cover_image}}" width="200px" height="100px" /></td></td>
                                                <td class="table-tit">{{ $row->destination_name }} </td>
                                                <td class="table-des">{{ $row->description }} </td>
                                            	<td>
                                            	    <a href="{{route('add-destination.edit',$row->id)}}" class="btn btn-warning waves-effect"><i class="far fa-edit"></i></a>
                                                    <form class="display-inline delete_form_{{$row->id}}" id="delete_form" method="POST" action="{{ route('add-destination.destroy',$row->id) }}">
    					                  {{ csrf_field() }}

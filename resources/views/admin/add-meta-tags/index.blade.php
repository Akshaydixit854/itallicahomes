                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header bg-pink">
                            <h2>Home Meta Tags</h2>
                            <a href="{{route('add-meta-tags.create')}}" class="btn btn-info m-t-15 waves-effect">Add New</a>
                        ...
                        <div class="body">
                            <div class="table-responsive">
                                <table class="display nowrap table table-bordered table-striped table-hover dataTable js-exportable" id="example">
                                    ...
                                    <tbody>
                                        @if (count($metatags) > 0)
                                        	@foreach($metatags as $key => $row)
                                            <tr>
                                            	<td>{{ $key + 1 }}</td>
                                                <td>{{ $row->meta_title }} </td>
                                                <td>{{ $row->page }} </td>
                                                <td>{{ substr($row->meta_description,0,200) }} </td>
                                                <td>{{ $row->meta_title_italian }} </td>
                                                <td>{{ substr($row->meta_desc_italian,0,200) }} </td>
                                                <td>{{ $row->meta_title_german }} </td>
                                                <td>{{ substr($row->meta_desc_german,0,200) }} </td>
                                            	<td>
                                            	    <a href="{{route('add-meta-tags.edit',$row->id)}}" class="btn btn-warning waves-effect"><i class="far fa-edit"></i></a>                                                    
                                                </td>
                                                <td>
                                            	    <form class="display-inline delete_form_{{$row->id}}" id="delete_form" method="POST" action="{{ route('add-meta-tags.destroy',$row->id) }}">
    					                  {{ csrf_field() }}

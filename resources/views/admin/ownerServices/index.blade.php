                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header bg-pink">
                            ...
                            <a href="{{url('')}}/admin/owner_create" class="btn btn-info m-t-15 waves-effect">Add New</a>
                        ...
                        <div class="body">
                            <div class="table-responsive">
                                <table class="display nowrap table table-bordered table-striped table-hover dataTable js-exportable" id="example">
                                    ...
                                    <tbody>
                                        @if (count($OwnerService) > 0)
                                        	@foreach($OwnerService as $key => $row)
                                            <tr>
                                            	<td>{{ $key+1 }}</td>
                                                <td class="table-prev-img"><img src={{asset("images/ownerService-cover-images/" . $row->cover_image)}} width="200px" height="100px"></td>
                                                <td>{!! substr($row->content,500) !!} </td>
                                                <td>{{ $row->published_by }} </td>
                                                <td>{{ $row->created_at }} </td>
                                                <td>@if($row->is_available == 1){{ 'Online' }} @else {{'Offline'}} @endif</td>
                                            	<td>
                                            		<a href="{{url('')}}/admin/owner_edit/{{$row->id}}" class="btn btn-warning waves-effect"><i class="far fa-edit"></i></a>
                                                </td>
                                                <td>
                                                    <form id="delete_form" class="display-inline delete_form_{{$row->id}}" method="POST" action="{{url('')}}/admin/owner_delete/{{$row->id}}">
    					                      {{ csrf_field() }}

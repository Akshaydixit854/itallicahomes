                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header bg-pink">
                            <h2> Property Types</h2>
                            <a href="{{route('add-property-types.create')}}" class="btn btn-info m-t-15 waves-effect">Add New</a>
                                    ...
                                    <tbody>
                                        @if (count($types) > 0)
                                            ...
                                            <tr>
                                            	<td>{{ $key + 1 }}</td>
                                                <td>{{ $row->type_name }} </td>
                                                <td>{{ substr($row->description,0,400) }} </td>
                                                <td bgcolor="{{ $row->ui_color }}"> </td>
                                            	<td>
                                            	    <a href="{{route('add-property-types.edit',$row->id)}}" class="btn btn-warning waves-effect"><i class="far fa-edit"></i></a>                                                    
                                                </td>
                                                <td>
                                            	    <form class="display-inline delete_form_{{$row->id}}" id="delete_form" method="POST" action="{{ route('add-property-types.destroy',$row->id) }}">
    					                  {{ csrf_field() }}

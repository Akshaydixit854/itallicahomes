                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header bg-pink">
                            <h2>New Amenity</h2>
                            <a href="{{route('add-amenities.create')}}" class="btn btn-info m-t-15 waves-effect">Add New</a>
                            ...
                            <div class="table-responsive">
                                <table class="display nowrap table table-bordered table-striped table-hover dataTable js-exportable" id="example">
                                    <thead>
                                        <tr>
                                            <th>@lang('app.id')</th>
                                            <th>@lang('app.amenities-name')</th>
                                            <th>@lang('app.amenities-display-name')</th>
                                            <th>Actions</th>
                                            <th>Actions</th>
                                        </tr>
                                    </thead>
                                    ...
                                    <tbody>
                                        @if (count($amenitiess) > 0)
                                        	@foreach($amenitiess as $row)
                                            <tr>
                                            	<td>{{ $row->id }}</td>
                                                <td>{{ $row->amenities_name }} </td>
                                                <td>{{ $row->amenities_display_name }} </td>
                                            	<td>
                                            	    <a href="{{route('add-amenities.edit',$row->id)}}" class="btn btn-warning waves-effect"><i class="far fa-edit"></i></a>
                                                </td>
                                                <td>
                                                    <form class="display-inline delete_form_{{$row->id}}" id="delete_form" method="POST" action="{{ route('add-amenities.destroy',$row->id) }}">
    					                 {{ csrf_field() }}
    					                 <input name="_method" type="hidden" value="DELETE">
                                        <button class="btn btn-danger waves-effect" onclick="delete_record('{{$row->id}}')"

...
@section('custom-css')
        <title>{{ __('app.buyer_services') }}</title>
@php
    if($buyerService ){
...
@endphp
    ...
    <div class="wrapper">
        <div class="about-page-banner">
            <img src={{asset("images/buyerService-cover-images/". $buyerService->cover_image)}} alt="" />
        </div>
        ...
        <div class="about-us-text-wrapper">
            <div class="main-container-wrapper">
                <span class="inner-blog-heading">
                    <input type="hidden" name="post_id" id="post_id" value="{{$buyerService->id}}">
            <input type="hidden" name="post_img" id="post_img" value={{asset("images/post-cover-images/". $buyerService->cover_image)}}><a href="javascript:history.back()"><svg width="18px" height="17px" viewBox="0 0 18 17" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="inner-blog-content" id="inner-blog-content">
                                  @if(\Lang::has('buyerServiceDesc.buyerService_description_'.$buyerService->id))
                                    {!! trans('buyerServiceDesc.buyerService_description_'.$buyerService->id) !!}
                                @else
                                    {!! $buyerService->content !!}
                                @endif
                            </div>
                        </div>
                    </div>
